...
def load_layers(filename, ignore=[], anchor_x=0, anchor_y=0,
        bezier_points=BEZIER_POINTS, circle_points=CIRCLE_POINTS):
    '''Load the layers from an SVG and return them in a dict of {name: SVG
    object}. The layer name is also available as .layer on the SVG object.
    '''
    ...
    gradients = GradientContainer()
    class svg_fake(object):
        gradients = GradientContainer()
    for e in root.getiterator('{http://www.w3.org/2000/svg}linearGradient'):
        svg_fake.gradients[e.get('id')] = LinearGradient(e, svg_fake) # SVG instance needed for parent stuff
    for e in root.getiterator('{http://www.w3.org/2000/svg}radialGradient'):
        svg_fake.gradients[e.get('id')] = RadialGradient(e, svg_fake) # SVG instance needed for parent stuff

    layers = {}
    for elem in root.getiterator('{http://www.w3.org/2000/svg}g'):
        label = elem.get('{http://www.inkscape.org/namespaces/inkscape}label')
        if not label or label in ignore: continue
        l = layers[label] = SVG.from_layer(filename, label, elem, anchor_x=anchor_x,
            anchor_y=anchor_y, bezier_points=bezier_points,
            circle_points=circle_points, gradients=svg_fake.gradients)
    return layers


class Path(object):
    def __init__(self, id, path, stroke, tris, fill, transform, label):
        self.id = id
        self.path = path
        self.stroke = stroke
        self.tris = tris
    ...
    def get_box(self):
        if self._box is None:
            ...
            width = max_x - min_x
            height = max_y - min_y
            self._box = rect.Rect(min_x, min_y, width, height)
        return self._box
...
class SVG(object):
    ...
    def __del__(self):
        glDeleteLists(self.disp_list, 1)

    @classmethod
    def load(cls, filename, anchor_x=0, anchor_y=0,
            bezier_points=BEZIER_POINTS, circle_points=CIRCLE_POINTS,
            gradients=None):
        """Loads an SVG object from a .svg or .svgz file.
        ...
        if (filename, bezier_points) in cls._disp_list_cache:
            ob.disp_list, ob.width, ob.height = cls._disp_list_cache[filename, bezier_points]
            return ob
        ...
        tree = parse(f)
        ob.parse_doc(tree)
        ob.disp_list = glGenLists(1)
        glNewList(ob.disp_list, GL_COMPILE)
        ob.render_slowly()
        glEndList()
        ob.anchor_x = anchor_x
        ob.anchor_y = anchor_y
        #cls._disp_list_cache[filename, bezier_points] = (ob.disp_list,
            #ob.width, ob.height)
        return ob

    @classmethod
    def from_layer(cls, filename, layer, tree, anchor_x=0, anchor_y=0,
            bezier_points=BEZIER_POINTS, circle_points=CIRCLE_POINTS,
            gradients=None):
        ob = cls(filename,
            bezier_points=bezier_points, circle_points=circle_points,
        ...
        key = (filename, layer, bezier_points)
        if key in cls._disp_list_cache:
            ob.disp_list, ob.width, ob.height = cls._disp_list_cache[key]
            return ob
        ...
        ob.anchor_x = anchor_x
        ob.anchor_y = anchor_y
        #cls._disp_list_cache[key] = (ob.disp_list, ob.width, ob.height)
        return ob
    ...
    def pop(self, id):
        ...
        Will cause a display list regeneration.
        '''
        path = self.paths_by_id.pop(id)
        self.paths.remove(path)
        if path.label in self.paths_by_label:
            self.paths_by_label[path.label].remove(path)
        self.dirty = True
        return path
    ...
    def render_slowly(self):
        ...
        for path in self.paths:
            ...
            if path.label and path.label[0] != '#':
                if path.label in self.paths_by_label:
                    self.paths_by_label[path.label].append(path)
                else:
                    self.paths_by_label[path.label] = [path]
            if path.tris:
                self.n_tris += len(path.tris)/3
                if isinstance(path.fill, str):
                    g = self.gradients[path.fill]
                    fills = [g.interp(x) for x in path.tris]
                else:
                    fills = [path.fill for x in path.tris]
                #pyglet.graphics.draw(len(tris), GL_TRIANGLES,
                #                     ('v3f', sum((x + [0] for x in tris), [])),
                #                     ('c3B', sum(fills, [])))
                glBegin(GL_TRIANGLES)
                for vtx, clr in zip(path.tris, fills):
                    vtx = path.transform(vtx)
                    glColor4ub(*clr)
                    glVertex3f(vtx[0], vtx[1], 0)
                glEnd()
            if path.stroke:
                for loop in path.path:
                    self.n_lines += len(loop) - 1
                    loop_plus = []
                    for i in xrange(len(loop) - 1):
                        loop_plus += [loop[i], loop[i+1]]
                    if isinstance(path.stroke, str):
                        g = self.gradients[path.stroke]
                        strokes = [g.interp(x) for x in loop_plus]
                    else:
                        strokes = [path.stroke for x in loop_plus]
    ...
    def parse_doc(self, tree):
        self.paths = []
        self.label = [None]
        self.width = self.parse_float(tree._root.get("width", '0'))
        self.height = self.parse_float(tree._root.get("height", '0'))
        if self.height:
            self.transform = Matrix([1, 0, 0, -1, 0, self.height])
        else:
            x, y, w, h = (self.parse_float(x) for x in parse_list(tree._root.get("viewBox")))
            self.transform = Matrix([1, 0, 0, -1, -x, h + y])
            self.height = h
            self.width = w
        self.opacity = 1.0
        for e in tree._root.getchildren():
            try:
    ...
    def parse_layer(self, layer):
        ...
        for e in layer.getchildren():
            try:
                ...
                print 'Exception while parsing element', e
                raise
        # figure width / height & anchor
        min_x = min_y = 999999999
        max_x = max_y = -999999999
        for path in self.paths:
            if not path.tris: continue
            for x, y in path.tris:
                min_x = min(x, min_x)
                max_x = max(x, max_x)
                min_y = min(y, min_y)
                max_y = max(y, max_y)
    ...
    def parse_element(self, e):
        ...
        if e.tag.endswith('path'):
            ...
            self.new_path()
            while pathdata:
                newop = pathdata.pop(0)

                if not newop.isalpha():
                    # repeat last opcode
                    pathdata.insert(0, newop)
                else:
                    opcode = newop
                ...
                if opcode == 'M':
                    ...
                    sweep = int(pathdata.pop(0))
                    x, y = pnext()
                    self.arc_to(rx, ry, phi, large_arc, sweep, mx + x, my + y)
            ...
            for orig_loop in self.path:
                ...
                for pt in orig_loop:
                    if (pt[0] - loop[-1][0])**2 + (pt[1] - loop[-1][1])**2 > TOLERANCE:
                        loop.append(pt)
                path.append(loop)
            self.paths.append(Path(self.id, path, self.stroke,
                               self.triangulate(path) if self.fill else None, self.fill,
                               self.transform, self.label[-1]))
        self.path = []
